...
class _RenderWrapper(torch.nn.Module):
    ...
    def forward(self, rays, mask, other_output, want_weights=False):
        if rays.shape[0] == 0:
            return (
        ...
        outputs = self.renderer(
            self.net, rays, mask, other_output, want_weights=want_weights and not self.simple_output
        )
        if self.simple_output:
            if self.renderer.using_fine:
                rgb = outputs.fine.rgb
                depth = outputs.fine.depth
                embed = None if "embed" not in outputs.fine else outputs.fine.embed
            else:
                rgb = outputs.coarse.rgb
                depth = outputs.coarse.depth
                embed = None if "embed" not in outputs.coarse else outputs.coarse.embed
            if embed is None:
                return rgb, depth
            else:
                return rgb, depth, embed
        else:
            # Make DotMap to dict to support DataParallel
            return outputs.toDict()


class NeRFEditingRenderer(torch.nn.Module):
    """
    NeRF differentiable renderer
    ...
    def __init__(
        ...
        self.regress_coord = regress_coord
        self.ret_last_feat = ret_last_feat
    ...
    def sample_fine(self, rays, weights):
        ...
        u = torch.rand(B, self.n_fine - self.n_fine_depth, dtype=torch.float32, device=device)  # (B, Kf)
    ...
    def composite(self, model, rays, z_samp, mask=None, other_outputs=None, coarse=True, sb=0):
        """
        ...
        with profiler.record_function("renderer_composite"):
            ...
            if use_viewdirs:
                ...
                for pnts, dirs in zip(split_points, split_viewdirs):
                    if not self.ret_last_feat:
                        ...
                        val, last_feat = model(pnts, coarse=coarse, viewdirs=dirs, ret_last_feat=True)
                        val_all.append(val)
            ...
            rgbs = out[..., :3]  # (B, K, 3)
            sigmas = out[..., 3]  # (B, K)
            if not self.regress_coord:
                embeds = out[..., 4:]
            else:
                embeds = out[..., 4:-3]
                coords = out[..., -3:]

            assert self.ret_last_feat
            ...
            if mask is not None:
                masked_embeds = embeds[mask[0]].reshape(-1, embeds.shape[-1])
                # masked_sigmas = sigmas[mask[0]].reshape(-1)
                norm_masked_embeds = masked_embeds / torch.maximum(
                    masked_embeds.norm(dim=-1, keepdim=True), torch.tensor(1e-6).to(masked_embeds.device)
                )
                max_indices = []
                embeds_begin = 0
                while embeds_begin < len(norm_masked_embeds):
                    embeds_end = min(embeds_begin + 750, len(norm_masked_embeds))
                    # temp_other_sigmas = other_outputs[:, 3]
                    # temp_sigmas_diff = torch.abs(masked_sigmas[embeds_begin:embeds_end, None] - temp_other_sigmas[None, :])
                    # temp_sigmas_mask = temp_sigmas_diff > 0.01
                    corr = torch.matmul(norm_masked_embeds[embeds_begin:embeds_end], other_outputs[:, 4:].T)
                    # corr[temp_sigmas_mask] = -1
                    max_indices.append(torch.argmax(corr, dim=-1))
                    embeds_begin = embeds_end

                if len(max_indices) > 0:
                    max_indices = torch.cat(max_indices, dim=0)
                    corr = None
                    norm_masked_embeds = None

                    embeds[mask[0]] = other_outputs[max_indices, 4:].reshape(-1, *embeds.shape[1:])
                    rgbs[mask[0]] = other_outputs[max_indices, :3].reshape(-1, *rgbs.shape[1:])
                    # sigmas[mask[0]] = other_outputs[max_indices, 3].reshape(sigmas[mask[0]].shape)
            ...
            if not self.regress_coord:
                ...
                return (
                    ...
                    coord_final,
                    depth_final,
                )  # make sure depth_final is the last one

    def sample_points(self, model, rays):
        ...
        points_coarse = rays[:, None, :3] + z_coarse.unsqueeze(2) * rays[:, None, 3:6]
        points_coarse = points_coarse.reshape(-1, 3)  # (B*K, 3)
        coarse_composite = self.composite(model, rays, z_coarse, coarse=True, sb=superbatch_size)
        coarse_weights = coarse_composite[0]

        if self.using_fine:
            fine_sampls = []
            if self.n_fine - self.n_fine_depth > 0:
                fine_sampls.append(self.sample_fine(rays, coarse_composite[0].detach()))  # (B, Kf - Kfd)
            if self.n_fine_depth > 0:
                fine_sampls.append(self.sample_fine_depth(rays, coarse_composite[-1]))  # (B, Kfd)
            z_fine = torch.cat(fine_sampls, dim=-1)  # (B, Kc + Kf)
            z_fine_sorted, argsort = torch.sort(z_fine, dim=-1)
            ...
            fine_weights = fine_composite[0]

            points_fine = rays[:, None, :3] + z_fine_sorted.unsqueeze(2) * rays[:, None, 3:6]
            points_fine = points_fine.reshape(-1, 3)  # (B*K, 3)

            return points_fine, fine_weights.reshape(-1)
        else:
            return points_coarse, coarse_weights.reshape(-1)
    ...
    def forward(
        ...
        with profiler.record_function("renderer_forward"):
            ...
            if self.using_fine:
                ...
                if self.n_fine_depth > 0:
                    all_samps.append(self.sample_fine_depth(rays, coarse_composite[-1]))  # (B, Kfd)
    ...
    @classmethod
    def from_conf(
        cls, conf, white_bkgd=False, lindisp=False, regress_coord=False, ret_last_feat=False, eval_batch_size=100000
    ):
        return cls(
