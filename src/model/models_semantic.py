...
class PixelNeRFSemanticNet(torch.nn.Module):
    def __init__(self, conf, stop_encoder_grad=False, cls_num=5):
        """
        ...
        self.linear_semantic_coarse = torch.nn.Linear(d_latent, cls_num)
        self.linear_semantic_fine = torch.nn.Linear(d_latent, cls_num)
        ...
        with profiler.record_function("model_inference"):
            ...
            if self.use_encoder:
                ...
                uv *= repeat_interleave(
                    self.focal.unsqueeze(1), NS if self.focal.shape[0] > 1 and self.focal.shape[0] != uv.shape[0] else 1
                )
                uv += repeat_interleave(
                    self.c.unsqueeze(1), NS if self.c.shape[0] > 1 and self.c.shape[0] != uv.shape[0] else 1
                )  # (SB*NS, B, 2)
            ...
            if coarse or self.mlp_fine is None:
                ...
                sem_logits = self.linear_semantic_coarse(last_feat)
                # sem_probs = self.soft_max(sem_logits)
            else:
                ...
                last_feat = mlp_output[..., self.d_out :]
                mlp_output = mlp_output[..., : self.d_out]
                sem_logits = self.linear_semantic_fine(last_feat)

            # Interpret the output
            # if ret_last_feat:
            mlp_output = mlp_output.reshape(-1, B, self.d_out)
            sem_logits = sem_logits.reshape(mlp_output.shape[0], B, -1)

            rgb = mlp_output[..., :3]
            sigma = mlp_output[..., 3:4]

            output_list = [torch.sigmoid(rgb), torch.relu(sigma), sem_logits]
