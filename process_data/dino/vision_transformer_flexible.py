    ...
    def get_specific_tokens(self, x, layers_to_return=(11,)):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i in layers_to_return:
                to_return[i] = {"t": x, "q": q, "k": k, "v": v, "attn": attn}
...
def preprocess(
    image_path: Union[str, Path] = None, pil_image: Image.Image = None, load_size: Union[int, Tuple[int, int]] = None
    ...
    if load_size is not None:
        if type(load_size) == int:
            pil_image = transforms.Resize(load_size, max_size=300, interpolation=transforms.InterpolationMode.LANCZOS)(
                pil_image
            )
        else:
